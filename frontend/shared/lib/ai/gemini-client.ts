...
export const FALLBACK_MODELS = ['gemini-3.6-flash', 'gemini-2.0-flash', 'gemini-1.5-flash'];
 ...
 * Strictly enforces AiProjectSchema.parse() validation on the response with typed error handling and automatic model fallback.
 */
export async function generateProjectWithAi(params: {
  ...
  const primaryModel = params.model || DEFAULT_MODEL;
  ...
  for (const currentModel of modelsToTry) {
    ...
    if (!response.ok) {
      ...
      // Si el modelo da 404 o no está disponible, intentar con el siguiente modelo de fallback
      if (response.status === 404 || errorDetail.includes('not found') || errorDetail.includes('no longer available')) {
        continue;
      }
  ...
  throw new AIException(
    'UNKNOWN',
    `Error del servicio de IA (${lastStatus}): ${lastErrorDetail || 'Modelo no disponible'}`,
    lastErrorDetail
  );
}
